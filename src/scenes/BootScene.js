...
preload() {
    // Texturas estáticas do Alex (8 no total)
    ['down', 'up', 'left', 'right'].forEach(dir => {
        this.load.image(`idle_${dir}`,  `assets/sprites/player/idle_${dir}.png`);
        this.load.image(`walk_${dir}`,  `assets/sprites/player/walk_${dir}.png`);
    });
    ...
    create() {
        ...
        // NOVA FUNÇÃO: Gera os móveis em HD para a Biblioteca
        this._gerarTexturasHD(); 

        this.scene.start('MenuPrincipal');
    }

    _gerarTexturasHD() {
        // Gera uma textura de estante com livros coloridos
        const texEstante = this.textures.createCanvas('textura_estante', 16, 16);
        const ctxE = texEstante.getContext();
        ctxE.fillStyle = '#3e2312'; ctxE.fillRect(0, 0, 16, 16); // Fundo escuro da estante
        ctxE.fillStyle = '#5c3a21'; ctxE.fillRect(0, 0, 16, 2);  // Prateleira superior
        ctxE.fillRect(0, 14, 16, 2); // Prateleira inferior
        
        // Desenhando lombadas de livros aleatórios
        const cores = ['#9e2a2b', '#335c67', '#e09f3e', '#e8e0d0', '#4a4e69'];
        for(let i = 1; i < 15; i += Phaser.Math.Between(2, 4)) {
            ctxE.fillStyle = cores[Math.floor(Math.random() * cores.length)];
            let alturaLivro = Phaser.Math.Between(8, 12);
            ctxE.fillRect(i, 14 - alturaLivro, 2, alturaLivro);
        }
        texEstante.refresh();

        // Gera uma textura de mesa de madeira bonita
        const texMesa = this.textures.createCanvas('textura_mesa', 16, 16);
        const ctxM = texMesa.getContext();
        ctxM.fillStyle = '#7a5230'; ctxM.fillRect(0, 0, 16, 16);
        ctxM.fillStyle = '#5c3d1e'; ctxM.fillRect(0, 14, 16, 2); // Borda da mesa
        texMesa.refresh();
    }
